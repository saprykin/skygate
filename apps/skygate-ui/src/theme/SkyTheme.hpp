...
public:
    [[nodiscard]] static std::optional<SkyThemeDefinition>
    parseTheme(const QByteArray& jsonBytes, const QString& sourceDescription);
};

class SkyThemeRepository final {
public:
    explicit SkyThemeRepository(const QString& manifestResourcePath = QStringLiteral(":/themes/manifest.json"));
