...
bool readBoolSetting(QSettings& settings, const QString& key, const bool fallback)
{
    ...
    if (!parsedValue.has_value()) {
        qCWarning(skygateSettingsLog).noquote()
            << "Invalid boolean setting" << key << "value" << value.toString() << "- using fallback" << fallback;
        return fallback;
    }
    return *parsedValue;
}
...
double readDoubleSetting(QSettings& settings, const QString& key, const double fallback)
{
    ...
    if (!ok || !std::isfinite(value)) {
        qCWarning(skygateSettingsLog).noquote() << "Invalid numeric setting" << key << "value"
                                                << settings.value(key).toString() << "- using fallback" << fallback;
        return fallback;
    }
    return value;
}
...
int readIntSetting(QSettings& settings, const QString& key, const int fallback)
{
    ...
    if (!ok) {
        qCWarning(skygateSettingsLog).noquote() << "Invalid integer setting" << key << "value"
                                                << settings.value(key).toString() << "- using fallback" << fallback;
        return fallback;
    }
    return value;
...
qulonglong readULongLongSetting(QSettings& settings, const QString& key, const qulonglong fallback)
{
    if (!settings.contains(key)) {
        return fallback;
    }
    ...
    if (!ok) {
        qCWarning(skygateSettingsLog).noquote() << "Invalid unsigned integer setting" << key << "value"
                                                << settings.value(key).toString() << "- using fallback" << fallback;
        return fallback;
    }
    return value;
...
QString readNonBlankStringSetting(QSettings& settings, const QString& key, const QString& fallback)
{
