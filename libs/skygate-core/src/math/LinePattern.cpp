...
DashedLineBuilder::build(const LineSegment2d& segment, const double dashLength, const double gapLength) const
{
    ...
    if (length <= 1e-9 || !std::isfinite(length) || dashLength <= 0.0 || gapLength < 0.0) {
        return segments;
    }
