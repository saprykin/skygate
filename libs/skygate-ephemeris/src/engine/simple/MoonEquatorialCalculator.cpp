...
core::EquatorialCoordinate MoonEquatorialCalculator::compute(const core::UtcTimePoint& utcTime) const noexcept
{
    ...
    const double meanAnomalyDeg = core::AngleMath::normalizeDegrees(115.3654 + 13.0649929509 * daysSinceJ2000);

    const double meanAnomalyRad = core::AngleMath::toRadians(meanAnomalyDeg);
    const double eccentricAnomalyDeg =
        meanAnomalyDeg
        + core::AngleMath::toDegrees(
    ...
    const double eclipticLongitudeDeg =
        core::AngleMath::normalizeDegrees(core::AngleMath::toDegrees(std::atan2(yH, xH)));
    const double eclipticLatitudeDeg = core::AngleMath::toDegrees(std::atan2(zH, std::sqrt(xH * xH + yH * yH)));
