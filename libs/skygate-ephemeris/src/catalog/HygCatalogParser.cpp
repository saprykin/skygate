...
struct BrightnessSelectionKey {
    double visualMagnitude = 0.0;
    std::size_t sequence = 0;
};

struct BrightnessSelectionCompare {
    [[nodiscard]] bool operator()(
        const BrightnessSelectionKey& lhs,
        const BrightnessSelectionKey& rhs
    ) const noexcept
    {
...
) const
{
    ...
    std::multimap<BrightnessSelectionKey, CelestialBody, BrightnessSelectionCompare> selectedBodies;
    ...
    std::size_t selectionSequence = 0;
        ...
        if (!line.isEmpty()) {
            ...
            if (!hasHeader) {
                ...
                if (raHours.has_value() && decDeg.has_value() && magnitude.has_value()) {
                    ...
                    bool shouldInsertBody = true;
                    if (selectedBodies.size() >= kMaxHygBodyCount) {
                        const auto dimmestSelected = std::prev(selectedBodies.end());
                        if (body.visualMagnitude >= dimmestSelected->first.visualMagnitude) {
                            shouldInsertBody = false;
                        } else {
                            selectedBodies.erase(dimmestSelected);
                        }
                    }

                    if (shouldInsertBody) {
                        selectedBodies.emplace(
                            BrightnessSelectionKey {
    ...
    bodies.reserve(selectedBodies.size());
    for (auto& [selectionKey, body] : selectedBodies) {
        (void) selectionKey;
