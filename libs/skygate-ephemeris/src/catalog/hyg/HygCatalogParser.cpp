...
) const
{
        ...
        [&](const DelimitedCatalogReader::Row& row, CatalogBodyParseResult& rowResult) {
            ...
            if (!raHours.has_value() || !decDeg.has_value() || !magnitude.has_value()) {
                ...
                if (invalidNumericRowSamples.size() < static_cast<qsizetype>(kMaxInvalidRowSamples)) {
                    invalidNumericRowSamples.push_back(QStringLiteral(
                        "row %1 ra='%2' dec='%3' mag='%4'"
                    ).arg(
    ...
    if (invalidNumericRowCount > 0U) {
        qCWarning(skygateCatalogParseLog).noquote()
            ...
            << "rows with invalid numeric values; samples:"
            << invalidNumericRowSamples.join(QStringLiteral("; "));
    }
