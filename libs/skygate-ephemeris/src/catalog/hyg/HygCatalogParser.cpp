...
HygCatalogParser::parse(const std::string_view csvData, const HygParseProgressCallback& progressCallback) const
{
    ...
    std::size_t invalidNumericRowCount = 0;
    QStringList invalidNumericRowSamples;
    ...
    CatalogBodyParseResult result = DelimitedCatalogReader::read(
        ...
        [&](const DelimitedCatalogRow& row, CatalogBodyParseResult& rowResult) {
            ...
            if (!raHours.has_value() || !decDeg.has_value() || !magnitude.has_value()) {
                ...
                if (invalidNumericRowSamples.size() < static_cast<qsizetype>(kMaxInvalidRowSamples)) {
                    invalidNumericRowSamples.push_back(
                        QStringLiteral("row %1 ra='%2' dec='%3' mag='%4'")
                            .arg(QString::number(static_cast<qulonglong>(dataRowNumber + 1U)), raText, decText, magText)
                    );
                }
                return true;
            }
    ...
    if (invalidNumericRowCount > 0U) {
        qCWarning(skygateCatalogParseLog).noquote()
            << "HYG CSV skipped" << static_cast<qulonglong>(invalidNumericRowCount)
            << "rows with invalid numeric values; samples:" << invalidNumericRowSamples.join(QStringLiteral("; "));
    }
