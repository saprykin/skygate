...
[[nodiscard]] std::optional<core::EquatorialCoordinate> constellationEquatorialById(const std::string& id)
{
    constexpr std::array<FixedStarCoordinate, 10> kConstellationCoordinates = {{
    ...
    for (const auto& constellation : kConstellationCoordinates) {
...
)
{
    ...
    if (body.type == CelestialBodyType::Constellation) {
        return constellationEquatorialById(body.id);
    }
