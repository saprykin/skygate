...
void CatalogArchiveZipDirectoryTests::directoryReaderRejectsMalformedStructures()
{
    ...
    QVERIFY(!skygate::ephemeris::ZipDirectoryReader::readEntries(truncatedCentralDirectory).has_value());
    ...
    QVERIFY(!skygate::ephemeris::ZipDirectoryReader::readEntries(invalidCentralSignature).has_value());
}
