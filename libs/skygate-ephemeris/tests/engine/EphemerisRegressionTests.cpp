...
void EphemerisRegressionTests::solarSystemBodiesMatchGoldenApproximation()
{
    ...
    for (const auto& expectedBody : expectedBodies) {
        ...
        compareEquatorial(state->equatorial, expectedBody.rightAscensionHours, expectedBody.declinationDeg);
    }
}
...
void EphemerisRegressionTests::solarSystemBodiesMatchGoldenApproximationAcrossContexts()
{
    ...
    for (const ExpectedContext& expectedContext : expectedContexts) {
        ...
        for (const ExpectedBody& expectedBody : expectedContext.bodies) {
            ...
            QVERIFY2(
                isNear(state->equatorial.rightAscensionHours, expectedBody.rightAscensionHours, 1e-9),
                expectedContext.label
            );
            QVERIFY2(
                isNear(state->equatorial.declinationDeg, expectedBody.declinationDeg, 1e-9), expectedContext.label
            );
            QVERIFY2(isNear(state->horizontal.altitudeDeg, expectedBody.altitudeDeg, 1e-9), expectedContext.label);
            QVERIFY2(isNear(state->horizontal.azimuthDeg, expectedBody.azimuthDeg, 1e-9), expectedContext.label);
        }
    }
}
...
void EphemerisRegressionTests::solarSystemBodiesStayNearExternalReferenceValues()
{
    ...
    for (const ExternalReference& reference : references) {
        ...
        const double raDeltaHours = std::abs(state->equatorial.rightAscensionHours - reference.rightAscensionHours);
        const double wrappedRaDeltaHours = std::min(raDeltaHours, 24.0 - raDeltaHours);
        QVERIFY2(wrappedRaDeltaHours * 15.0 <= reference.equatorialToleranceDeg, reference.id);
        QVERIFY2(
            isNear(state->equatorial.declinationDeg, reference.declinationDeg, reference.equatorialToleranceDeg),
            reference.id
        );
        QVERIFY2(
            isNear(state->horizontal.altitudeDeg, reference.altitudeDeg, reference.horizontalToleranceDeg), reference.id
        );
        QVERIFY2(
            isNear(state->horizontal.azimuthDeg, reference.azimuthDeg, reference.horizontalToleranceDeg), reference.id
        );
    }
}
...
void EphemerisRegressionTests::eclipticToEquatorialPreservesSeasonalAnchors()
{
    ...
    const auto vernalEquinox = skygate::ephemeris::EclipticToEquatorialCalculator::compute(0.0, 0.0, kObliquityDeg);
    const auto northernSolstice = skygate::ephemeris::EclipticToEquatorialCalculator::compute(90.0, 0.0, kObliquityDeg);
    const auto southernSolstice =
        skygate::ephemeris::EclipticToEquatorialCalculator::compute(270.0, 0.0, kObliquityDeg);
...
void EphemerisRegressionTests::rightAscensionWrapsAcrossTwentyFourHours()
{
    ...
    const auto wrappedRightAscension = skygate::ephemeris::EquatorialToHorizontalCalculator::compute(
...
void EphemerisRegressionTests::negativeLongitudeMirrorsPositiveLongitude()
{
    ...
    const auto westLongitudeHorizontal = skygate::ephemeris::EquatorialToHorizontalCalculator::compute(
        {.rightAscensionHours = greenwichRightAscensionHours, .declinationDeg = 0.0},
        {.latitudeDeg = 0.0, .longitudeDeg = -15.0, .elevationMeters = 0.0},
        j2000NoonUtc
    );
...
void EphemerisRegressionTests::polarObserverPlacesMatchingCelestialPoleAtZenith()
{
    ...
    const auto northPoleHorizontal = skygate::ephemeris::EquatorialToHorizontalCalculator::compute(
        {.rightAscensionHours = 12.0, .declinationDeg = 90.0},
        {.latitudeDeg = 90.0, .longitudeDeg = 0.0, .elevationMeters = 0.0},
        j2000NoonUtc
    );
    const auto southPoleHorizontal = skygate::ephemeris::EquatorialToHorizontalCalculator::compute(
        {.rightAscensionHours = 12.0, .declinationDeg = -90.0},
        {.latitudeDeg = -90.0, .longitudeDeg = 0.0, .elevationMeters = 0.0},
        j2000NoonUtc
    );
