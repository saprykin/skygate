...
[[nodiscard]] OwnGalaxyCelestialBody makeStarBody()
{
    OwnGalaxyCelestialBody body;
    body.id = "vega";
...
[[nodiscard]] OwnGalaxyCelestialBody makeAstrometricStarBody(std::string id, const double rightAscensionHours)
{
    OwnGalaxyCelestialBody body = makeFixedStarBody(std::move(id), rightAscensionHours, 10.0);
...
void mergeConversionMetadata(EphemerisEngineQueryResult& metadata, const TimeScaleConversionResult& conversion) noexcept
{
...
private slots:
    ...
    void reusesCachedSingleBodyStatesWithoutFullFrameSnapshots();
...
void HighPrecisionEphemerisEngineTests::exposesMetadataAndCapabilities()
{
    ...
    const HighPrecisionEphemerisEngine engine(makeCatalog(bodies), options, std::move(dependencies));
...
void HighPrecisionEphemerisEngineTests::usesBatchStarPathForFullFrameSnapshot()
{
    ...
    const EphemerisSnapshot firstSnapshot = engine.compute(firstRequest);
    ...
    const EphemerisSnapshot secondSnapshot = engine.compute(secondRequest);
...
void HighPrecisionEphemerisEngineTests::batchesRepresentativeLargeCatalogApparentPlaceTransformsOnce()
{
    ...
    const HighPrecisionEphemerisEngine engine(
        makeCatalog(bodies),
        makeRequest().options,
        makeDependencies({}, starAstrometryCalculator, apparentPlaceCalculator)
    );
...
void HighPrecisionEphemerisEngineTests::batchesTopocentricApparentPlaceRequestWideState()
{
    ...
    const HighPrecisionEphemerisEngine engine(
        makeCatalog(bodies), request.options, makeDependencies({}, starAstrometryCalculator, apparentPlaceCalculator)
    );
...
void HighPrecisionEphemerisEngineTests::reusesCachedFullFrameSnapshotsWithoutStaleResults()
{
    ...
    firstRequest.epoch.julianDatePart2 = 0.25;
    const EphemerisSnapshot firstSnapshot = engine.compute(firstRequest);
    const EphemerisSnapshot repeatedSnapshot = engine.compute(firstRequest);
    ...
    const EphemerisSnapshot changedSnapshot = engine.compute(changedRequest);
...
{
    const std::array bodies{makeSunBody()};
    auto solarSystemCalculator = std::make_shared<ThreadSafeEpochSolarSystemCalculator>(9.0);
    auto computationCache = std::make_shared<EphemerisComputationCache>();
    ...
    const auto firstState = engine.computeBodyState(request, std::size_t{0});
    const auto repeatedState = engine.computeBodyState(request, std::size_t{0});

    QVERIFY(firstState.has_value());
    QVERIFY(repeatedState.has_value());
    QCOMPARE(solarSystemCalculator->callCount(), 1);
    QCOMPARE(firstState->equatorial.rightAscensionHours, 9.125);
    QCOMPARE(repeatedState->equatorial.rightAscensionHours, 9.125);

    EphemerisRequest changedRequest = request;
    changedRequest.epoch.julianDatePart2 = 0.375;
    const auto changedState = engine.computeBodyState(changedRequest, std::size_t{0});

    QVERIFY(changedState.has_value());
    QCOMPARE(solarSystemCalculator->callCount(), 2);
    QCOMPARE(changedState->equatorial.rightAscensionHours, 9.375);

    const auto originalState = engine.computeBodyState(request, std::string_view{"sun"});

    QVERIFY(originalState.has_value());
    QCOMPARE(solarSystemCalculator->callCount(), 2);
    QCOMPARE(originalState->equatorial.rightAscensionHours, 9.125);
}

void HighPrecisionEphemerisEngineTests::isolatesCachedSingleBodyStatesByBodyIndex()
{
    const std::array bodies{makeSunBody(), makeSunBody()};
    auto solarSystemCalculator = std::make_shared<ThreadSafeEpochSolarSystemCalculator>(11.0);
    auto computationCache = std::make_shared<EphemerisComputationCache>();
    ...
    const auto firstState = engine.computeBodyState(request, std::size_t{0});
    const auto secondState = engine.computeBodyState(request, std::size_t{1});
    const auto repeatedFirstState = engine.computeBodyState(request, std::size_t{0});

    QVERIFY(firstState.has_value());
    QVERIFY(secondState.has_value());
    QVERIFY(repeatedFirstState.has_value());
    QCOMPARE(solarSystemCalculator->callCount(), 2);
    QCOMPARE(firstState->bodyIndex, std::uint32_t{0});
    QCOMPARE(secondState->bodyIndex, std::uint32_t{1});
    QCOMPARE(repeatedFirstState->bodyIndex, std::uint32_t{0});
}
...
void HighPrecisionEphemerisEngineTests::servesConcurrentReadOnlyComputationsFromCache()
{
    ...
    EphemerisRequest request = makeRequest();
    request.epoch.julianDatePart2 = 0.125;
    const EphemerisSnapshot warmedSnapshot = engine.compute(request);
...
void HighPrecisionEphemerisEngineTests::isolatesCachedSnapshotsByDataSetRevision()
{
    ...
    const std::vector<OwnGalaxyCelestialBody> bodies{makeSunBody()};
    ...
    EphemerisSnapshot oldSnapshot;
    oldSnapshot.states.push_back(
        CelestialBodyState{
            .bodyIndex = 0U,
            .equatorial = {.rightAscensionHours = 1.0, .declinationDeg = 2.0},
        }
    );
    EphemerisSnapshot newSnapshot;
    newSnapshot.states.push_back(
        CelestialBodyState{
            .bodyIndex = 0U,
...
void HighPrecisionEphemerisEngineTests::isolatesCachedSnapshotsByMicrosecondUtc()
{
    ...
    EphemerisSnapshot baseSnapshot;
    ...
    EphemerisSnapshot shiftedSnapshot;
...
void HighPrecisionEphemerisEngineTests::isolatesCachedSnapshotsByDataSetDateRangeContents()
{
    ...
    oldDataSet.dateRanges.push_back(
        EphemerisDateRange{
            .id = "modern",
            .displayName = "Modern",
            .start = {.julianDatePart1 = 2'400'000.5, .julianDatePart2 = 0.0, .timeScale = TimeScale::Tdb},
...
{
    EphemerisComputationCache computationCache;
    std::vector<OwnGalaxyCelestialBody> bodies{makeFixedStarBody("star-a", 1.0, 2.0)};
    const CelestialBodyCatalog oldCatalog(bodies);
    const EphemerisRequest request = makeRequest();
...
void HighPrecisionEphemerisEngineTests::forwardsOptionsThroughCollaboratorsAndResultBuilder()
{
    ...
    const HighPrecisionEphemerisEngine engine(
        makeCatalog(bodies),
        engineOptions,
        makeDependencies(solarSystemCalculator, {}, apparentPlaceCalculator, resultBuilder)
    );
...
void HighPrecisionEphemerisEngineTests::bypassesApparentPlaceForGeometricStarRequests()
{
    ...
    const HighPrecisionEphemerisEngine engine(
        makeCatalog(bodies),
        request.options,
        makeDependencies({}, starAstrometryCalculator, apparentPlaceCalculator, resultBuilder)
    );
...
void HighPrecisionEphemerisEngineTests::validatesRequestsBeforeDispatchingCalculators()
{
    ...
    HighPrecisionEphemerisEngine engine(
        makeCatalog(bodies), makeRequest().options, makeDependencies(solarSystemCalculator, {}, {}, resultBuilder)
    );
...
void HighPrecisionEphemerisEngineTests::defaultResultBuilderPreservesFailedResultsWithoutFallback()
{
    ...
    calculatorResult.metadata.addWarning(EphemerisEngineWarning::Code::MissingEphemerisData);
...
void HighPrecisionEphemerisEngineTests::defaultResultBuilderPreservesDegradedDataWarnings()
{
    ...
    calculatorResult.metadata.status = skygate::ephemeris::EphemerisEngineQueryStatus::Type::Degraded;
    calculatorResult.metadata.addWarning(EphemerisEngineWarning::Code::AccuracyDegraded);
    calculatorResult.metadata.addWarning(EphemerisEngineWarning::Code::TimeScaleDataUnavailable);
    calculatorResult.metadata.addWarning(EphemerisEngineWarning::Code::MissingEphemerisData);
    ...
    QVERIFY(state->metadata.hasWarning(EphemerisEngineWarning::Code::AccuracyDegraded));
